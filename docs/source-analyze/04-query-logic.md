...
class CallProcessor:
    def process_calls_in_file(
        ...
        # CALLS 的 from_id 必须为 Function 或 Method，故优先解析 functions 域与 method 域
        # 1. 处理函数域内的调用（caller = FUNCTION）
        self._process_calls_in_functions(...)

        # 2. 处理方法域内的调用（caller = METHOD，类内方法）
        self._process_calls_in_methods(...)

        # 不再解析 module 级 CALLS：schema 要求 caller 为 Function/Method，且会错误归因本属于 Method 的调用
```
